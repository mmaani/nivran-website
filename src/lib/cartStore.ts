...
export function normalizeCartItems(items: unknown): CartItem[] {
  ...
  for (const it of items) {
    ...
    const variantId = Math.max(0, Math.trunc(toNum((it as JsonRecord).variantId)));
    if (!slug || !variantId) continue;

    out.push({
      slug,
      variantId,
      variantLabel: toStr((it as JsonRecord).variantLabel).trim(),
  ...
  for (const i of out) map.set(`${i.slug}::${i.variantId}`, i);
...
export function mergeCartSum(a: CartItem[], b: CartItem[]): CartItem[] {
  ...
  for (const it of normalizeCartItems(a)) map.set(`${it.slug}::${it.variantId}`, { ...it });

  for (const it of normalizeCartItems(b)) {
    const key = `${it.slug}::${it.variantId}`;
