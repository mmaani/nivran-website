  ...
  try {
    ...
    return parsed
      .map((x: unknown) => {
        ...
        variantId: Number.isFinite(Number(item.variantId || 0)) && Number(item.variantId || 0) > 0 ? Math.floor(Number(item.variantId || 0)) : null,
...
export default function CartClient({ locale }: { locale: Locale }) {
  ...
  function inc(slug: string, variantId: number | null) {
    const next = items.map((i) => (i.slug === slug && (i.variantId||null)===(variantId||null) ? { ...i, qty: Math.min(99, i.qty + 1) } : i));
    setAndSync(next);
  }

  function dec(slug: string, variantId: number | null) {
    const next = items.map((i) => (i.slug === slug && (i.variantId||null)===(variantId||null) ? { ...i, qty: Math.max(1, i.qty - 1) } : i));
    setAndSync(next);
  }

  function remove(slug: string, variantId: number | null) {
    const next = items.filter((i) => !(i.slug === slug && (i.variantId||null)===(variantId||null)));
    setAndSync(next);
  }
              ...
              >
                <div>
                  ...
                  <div className="muted" style={{ marginTop: 4 }}>
                    {i.slug} {i.variantLabel ? `· ${i.variantLabel}` : ""}
                  </div>
                  <div className="muted" style={{ marginTop: 4 }}>
