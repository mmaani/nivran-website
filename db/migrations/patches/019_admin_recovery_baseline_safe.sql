-- Admin Recovery Baseline (SAFE) for Neon / managed Postgres
-- Use this instead of 017 when prior runs fail with "ROLLBACK required".
--
-- Why this exists:
-- - 017 can fail if legacy data violates new constraints (e.g. unknown staff roles)
-- - 017 can fail on unique index creation if duplicate newsletter emails exist
-- - once one statement fails in a transaction, Neon requires ROLLBACK before continuing
--
-- Pre-step in Neon SQL editor if needed:
--   ROLLBACK;

-- Keep this script mostly non-transactional so one non-critical failure doesn't poison all steps.
...
-- De-duplicate newsletter emails before unique index creation.
-- Keep oldest row per normalized email.
with ranked as (
  select id,
...
delete from newsletter_subscribers n
using ranked r
where n.id = r.id
  and r.rn > 1;
...
-- Legacy compat: if table has email but no username, copy values.
do $$
begin
  if exists (
    ...
    execute 'update staff_users set username = coalesce(username, email)';
  end if;
exception when others then
  null;
end $$;

-- Normalize weird roles to sales to avoid check-constraint failures.
update staff_users
set role = case
  when lower(coalesce(role,'')) in ('admin','ops','sales','staff') then lower(role)
  else 'sales'
end;

-- Ensure required non-nulls for legacy rows.
update staff_users set username = coalesce(nullif(username,''), concat('staff_', id::text, '@local'));
update staff_users set password_hash = coalesce(nullif(password_hash,''), 'RESET_REQUIRED');
